...
import { isBoolean, isComplex, isConstructed, isEmpty, isExact, isExactNonnegativeInteger, isInexact, isInexactFloat, isInstance, isInteger, isList, isNatural, isNumber, isRational, isReal, isString, isSymbol, RacketComplexNumber, RacketConstructedList, RacketExactNumber, RacketInexactFraction, RacketString, RacketStructure, RACKET_EMPTY_LIST, RACKET_FALSE, RACKET_TRUE } from './values.js';
/* -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
 ...
 * Abstract Class
 * -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  - */
export class RacketBuiltInFunction {
...
class SymStar extends RacketBuiltInFunction {
    ...
    call(args) {
        ...
        let result = new RacketExactNumber(1n, 1n);
...
class SymPlus extends RacketBuiltInFunction {
    ...
    call(args) {
        super.call(args);
        let numbers = assertListOfNumbers(this.name, args);
        let result = new RacketExactNumber(0n, 1n);
        for (let number of numbers) {
 ...
 */
function assertAtLeastNArguments(name, minimum, received) {
    if (minimum <= received) {
        return;
    }
    let errMsg = name + ': ';
 ...
 */
function assertListOfLengthAtLeastN(name, minimum, received) {
    let list = received;
 ...
 * @param received the number of arguments received
 */
function assertExactlyNArguments(name, expected, received) {
 ...
 */
function assertRangeOfArguments(name, min, max, args) {
 ...
 * @param args the received arguments
 */
function assertSecondList(name, args) {
    let list = args[1];
    if (!isList(list)) {
        // @ts-ignore
 ...
 */
function assertNthExactNonnegativeInteger(name, arg, nth) {
 ...
 * @param nth the position of the argument
 */
function assertNthStrNatural(name, arg, nth) {
    if (!isNatural(arg)) {
 ...
 * @param nth the position of the argument
 */
function assertNthStrString(name, arg, nth) {
    if (!isString(arg)) {
        error(`${name}: expected a natural number for the ${nthString(nth)} argument, but received ${arg.toString()}`);
    }
    return arg;
 ...
 * @param args the received arguments
 */
function assertLastList(name, args) {
 ...
 * @param args the received arguments
 */
function assertListOfIntegers(name, args) {
    let integers = [];
 ...
 * @param args the received arguments
 */
function assertListOfNumbers(name, args) {
    let numbers = [];
    args.forEach((arg, idx) => {
        if (!isNumber(arg)) {
            if (args.length === 1) {
 ...
 * @param args the received arguments
 */
function assertListOfReals(name, args) {
    let numbers = [];
    args.forEach((arg, idx) => {
        if (!isReal(arg)) {
            if (args.length === 1) {
 ...
 * @param args the received arguments
 */
function assertListOfBooleans(name, args) {
    let booleans = [];
    args.forEach((arg, idx) => {
        if (!isBoolean(arg)) {
            if (args.length === 1) {
 ...
 * @param args the received arguments
 */
function assertListOfSymbols(name, args) {
    let symbols = [];
    args.forEach((arg, idx) => {
        if (!isSymbol(arg)) {
            if (args.length === 1) {
 ...
 * @param args the received arguments
 */
function assertListOfStrings(name, args) {
    let strings = [];
    args.forEach((arg, idx) => {
        if (!isString(arg)) {
            if (args.length === 1) {
 ...
 * -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  - */
function addBuiltinFunction(fun) {
    BUILT_INS.set(fun.name, fun);
}
function addBuiltInFunctionAlias(fun, alias) {
    BUILT_INS.set(alias, fun);
    fun.name = alias;
}
function addBuiltInStructure(structure) {
